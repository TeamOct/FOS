...
public class FOSOreRenderer {
    ...
    public FOSOreRenderer(){
        ...
        Events.run(EventType.Trigger.draw, this::drawOre);
        Events.on(EventType.TileChangeEvent.class, event -> recacheTile(event.tile));
    }
    ...
    public void drawOre(){
        ...
        int layers = oreCacheLayers.size;
        ...
        for(int i = 0; i < drawnLayers.size; i++){
            CacheLayer layer = oreCacheLayers.get(drawnLayers.get(i));
    ...
    private void cacheChunk(int cx, int cy){
        ...
        for(int tilex = Math.max(cx * chunkSize - 1, 0); tilex < (cx + 1) * chunkSize + 1 && tilex < world.width(); tilex++){
            for(int tiley = Math.max(cy * chunkSize - 1, 0); tiley < (cy + 1) * chunkSize + 1 && tiley < world.height(); tiley++){
                ...
                if (tile.overlay() instanceof OreBlock && tile.overlay().cacheLayer instanceof AnimatedOreCacheLayer layer)
                    used.add(layer);
            }
        }

        if(cache[cx][cy].length == 0){
            cache[cx][cy] = new Mesh[oreCacheLayers.size];
        }

        var meshes = cache[cx][cy];

        for(CacheLayer layer : oreCacheLayers){
    ...
    private Mesh cacheChunkLayer(int cx, int cy, CacheLayer layer){
        ...
        for(int tilex = cx * chunkSize; tilex < (cx + 1) * chunkSize; tilex++){
            for(int tiley = cy * chunkSize; tiley < (cy + 1) * chunkSize; tiley++){
                ...
                Floor overlay;
                if (tile != null && (overlay = tile.overlay()) != null && overlay.cacheLayer == layer) {
    ...
    public void clearTiles(){
        ...
        cache = new Mesh[chunksx][chunksy][dynamic ? 0 : oreCacheLayers.size];
    ...
    class FOSOreRendererBatch extends Batch{
        @Override
